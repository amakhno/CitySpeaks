...
BEGIN  
    ...
    INSERT INTO Images (ImageData, ImageMimeType)		
		SELECT [ImageData], [ImageMimeType] from
			CitySpeaks_old.dbo.Workers where ID = @vendor_id
	...
	INSERT INTO Workers (Name, ShortDescription, FullDescription, SmallImageId, BigImageId)
		(SELECT Name, ShortDescription, FullDescription, @Image1Id, @Image2Id from
			CitySpeaks_old.dbo.Workers where ID = @vendor_id)  
    FETCH NEXT FROM vendor_cursor   
    INTO @vendor_id
END   
